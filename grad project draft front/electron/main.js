...
function runFirewallQuery() {
  return new Promise((resolve) => {
    const cmd = `powershell -NoProfile -Command "Get-NetFirewallProfile | Select-Object Name,Enabled | ConvertTo-Json -Compress"`
    exec(cmd, { timeout: 8000 }, (err, stdout) => {
      if (err || !stdout.trim()) {
        resolve({ pass: false })
        return
      ...
      try {
        let data = JSON.parse(stdout.trim())
        if (!Array.isArray(data)) data = [data]
        // Check if all profiles (Domain, Private, Public) are enabled
        const allEnabled = data.every(p => p.Enabled === 1 || p.Enabled === true)
        resolve({ pass: allEnabled, profiles: data })
      } catch {
        resolve({ pass: false })
      }
    })
...
function runDefenderQuery() {
  return new Promise((resolve) => {
    const cmd = `powershell -NoProfile -Command "Get-MpComputerStatus | Select-Object AMServiceEnabled,AntivirusEnabled,RealTimeProtectionEnabled | ConvertTo-Json -Compress"`
    exec(cmd, { timeout: 8000 }, (err, stdout) => {
      if (err || !stdout.trim()) {
        resolve({ pass: false })
        return
      ...
      try {
        ...
        const pass = data.AMServiceEnabled && data.RealTimeProtectionEnabled
        resolve({ pass: !!pass, detail: data })
      } catch {
        resolve({ pass: false })
      }
    })
...
ipcMain.handle('compliance:run', async () => {
  ...
  const fwResult = await runFirewallQuery()
  const dfResult = await runDefenderQuery()
  const diskFreeGb = osInfo.freeMem

  const pass = osBuild.pass && avResult.found && fwResult.pass && dfResult.pass && diskFreeGb >= 2
  ...
  return {
    ...
    firewall:  { pass: fwResult.pass },
    defender:  { pass: dfResult.pass },
